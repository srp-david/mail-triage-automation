...
문서 버전: 3.3 · 갱신일: 2026-09-23 · 상태: **Release 배포 기반 → 간편 설치·업데이트 → GitHub push·Release 게시 → 설치본 실분석 → 팀 파일럿 → 피드백·안정화 → AWS 이전 검토 → 원격 분석 → SR 구현·검증·PR 자동화**. 첫 Supabase API·DB 배포와 합성 종단 검증을 완료했다. Release/설치/업데이트 소스와 candidate.7 로컬 후보 검증이 진행됐고, 배너·로그아웃 겹침을 조정한 candidate.9를 최종 배포 후보로 선택했다. candidate.9 설치·UI 회귀 검증, GitHub 게시·hosted API 갱신·실분석은 아직 완료되지 않았다. 사용자 요청으로 백업 설정은 보류하며, 실제 두 PC 업무 파일럿과 M1 운영 준비 완료는 별도로 판정한다.
...
| 앱 배포 방향 | 사용자 생성 [srp-david/mail-triage-automation](https://github.com/srp-david/mail-triage-automation)을 사용. 2026-09-22 public·빈 저장소 확인. GitHub Release에 앱 파일·변경 내역 보관, 필요 시 private 전환. 소스/이력 공개·push·Release 게시는 별도 |
...
| Windows 배포·업데이트 | self-extracting setup.exe·실행/작업 중지/앱 종료 shortcut·서명 메타데이터·인증 update API·다운로드/updater·버튼 소스 추가. candidate.7 한글 설치·기동·중지·종료·업그레이드 로컬 검증, candidate.9 배너/로그아웃 UI 조정 | candidate.9 설치·업데이트·UI 회귀 검증, hosted API 갱신, GitHub push·Release 게시, 팀 PC 수용·실분석 대기 |
...
| D13 | 저장소는 `srp-david/mail-triage-automation`으로 확정(public·빈 저장소 확인). 공개 대상/이력 검토, stable/test 제공 정책·정식 패키징/승인·서명키/교체·게시 담당·최소 API 지원 기간·private 다운로드 방식은 남음 | Release 게시/업데이트 제공 전 | update API·UI·updater·서명/전환 모의 구현과 검증 |
...
|---|---|---|---|
| SET1 | setup.exe 빌더·실행/화면 열기·작업 중지·앱 종료 shortcut 소스 추가, candidate.7 한글 경로 로컬 검증. 최종 후보 candidate.9·깨끗한 PC 수용 대기 | M1~M2, D5/D6 | 설치·포트/한글 경로·실행/작업 중지/앱 종료 구분·설정 보존·롤백 |
...
| U1 | 서명 메타데이터·인증된 update API 소스 추가. hosted API 갱신·실계정 제공 검증 대기 | M1~M2, D13 | 사용자/채널·버전·호환·중단·서명 검증 |
| U2 | 로컬 다운로드/updater·업데이트 버튼 소스 추가. 게시 asset·실사용 업데이트 검증 대기 | M2, U1/SET1 | 작업 대기·중단 다운로드·재시작·롤백·개인 상태 보존 |
| U3 | setup.exe 빌더와 candidate.7 ZIP/설치본·서명 메타데이터 로컬 생성. candidate.9 최종 후보 선정, 패키징/게시 전 | M1~M3, D6/D13·공개 전 검토. 배포 계약·빌드 기반은 SET1/U2보다 먼저, 실제 push·게시는 설치 파일 검증 뒤 | 재현 빌드·검토된 소스 push·검증된 설치 파일 Release 게시 기록·기존 클라이언트 모의 전환 수용. 실제 private 전환은 필요 시 별도 |
